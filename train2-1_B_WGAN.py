        ...
        for epoch in range(num_epochs):
            ...
            for i, data in enumerate(dataloader, 0):
                ...
                if i % n_critic == 0:
                    ...
                    r_logits = netD(real_cpu + ratio*torch.normal(0, 1, (b_size, 3, image_size, image_size)).to(device)).view(-1)
                    r_logit_mean = torch.mean(r_logits)

                    ## Train with all-fake batch

                    # Classify all fake batch with D
                    f_logits = netD(fake.detach() + ratio*torch.normal(0, 1, (b_size, 3, image_size, image_size)).to(device) ).view(-1)
