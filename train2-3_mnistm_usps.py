...
# Train on source domain mnistm, eval on target domain svhn.
import os
import glob
import argparse
import random
...
def test(dataloader, my_net):
    ...
    while i < len_dataloader:
        ...
        class_output = my_net(input_data=t_img)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--ckpt_dir", help="Checkpoint location", default="ckpt2-3_mnistm_usps")
    ...
    img_transform_target = transforms.Compose([
        transforms.Resize(image_size),
        transforms.ToTensor(),
        transforms.Normalize(mean=(0.1307,), std=(0.3081,)) # usps is gray
    ])
    ...
    for epoch in range(n_epoch):
        ...
        len_dataloader =  len(train_dataloader_source)
        #data_source_iter = iter(train_dataloader_source)
        data_target_iter = iter(train_dataloader_source)
        ...
        for i in range(len_dataloader):
            ...
            #  We don't training model using target data
        ...
        print(f"err_s_label= {err_s_label.data.cpu().numpy()}")
        ...
        print(f'Accuracy of the target dataset {target_dataset_name}: {accu_t}')

        # Source domain (We don't do here.)
        # accu_s = test(val_dataloader_source, my_net)
        # print(f'Accuracy of the source dataset {source_dataset_name}: {accu_s}')
