...
if args.mode=="test":
    ...
    ckpt_path = os.path.join("ckpt2-1_pei", "G_144_simple.pth")
    ...
    #for i in range(n_generate):
    #    plt.imsave(os.path.join(output_dir,f'{i+1}.jpg') "./output_images_3/{0:03d}.png".format(i), imgs[i])
    print("Done.")
...
if args.mode=="train":
    ...
    resume_path = os.path.join("ckpt2-1_pei", "G_144_simple.pth")
    print(f"Load model from: {resume_path}")
    netG.load_state_dict(torch.load(resume_path))
    resume_path = os.path.join("ckpt2-1_pei", "D_144_simple.pth")
    print(f"Load model from: {resume_path}")
